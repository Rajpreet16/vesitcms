...
class FacultyController extends Controller
{
    ...
    public function facultyattendance(){
        if(session('e_id')){
            $e_id=session('e_id');
            $current_month_details = info::where('e_id','=',$e_id)
            ->whereYear('Date','=','2017')
            ->whereMonth('Date','=','01')
            ->get();
            
            
            return view('faculty.pages.facultyattendance')->with('data',$current_month_details);
    ...
    public function facultyattendanceajax(Request $request){
        ...
        if(session('e_id')){
            ...
            $start_date_of_month = (string)$y.'-'.(string)$m.'-01';
            $end_date_of_month = (string)$y.'-'.(string)$m.'-'.(string)$last_day;

            $array_of_working_slot = array(0 => 'T1', 1 => 'T2', 2 => 'T3', 3 => 'NT1', 4 => 'NT2', 5 => 'NT3');
            // $current_month_details = info::where('e_id','=',$e_id)
            // ->whereYear('Date','=',$y)
            // ->whereMonth('Date','=',$m)
            // ->get();

            $detail = $current_month_details;
            $slot = DB::table('staff')->where('e_id','=',$e_id)->value('slot');

            
            $path = '../storage/json/staffWorkingSlotConfig.json';
            $content = json_decode(file_get_contents($path), true);

            $index_in_json_file = array_search($slot, $array_of_working_slot);
            
            $start_time = $content["slot"][$index_in_json_file][$slot][0]['start_time'];
            $end_time = $content["slot"][$index_in_json_file][$slot][0]['end_time'];
            $first_late = $content["slot"][$index_in_json_file][$slot][0]['first_late'];
            $second_late = $content["slot"][$index_in_json_file][$slot][0]['second_late'];
            $absent = $content["slot"][$index_in_json_file][$slot][0]['absent'];
            ...
            for( $i = 0; $i< sizeof($detail); $i++){
                ...
                if ($detail[$i]["in_time"] >  $absent)
                {
                    $slot_status = 3;
                    $slot_color = "#ff6666";
                }
                else if ($detail[$i]["in_time"] <= $absent and $detail[$i]["in_time"]>=  $second_late){
                    $slot_status = 2;
                    $slot_color = "#ff80ff";
                }
                else if ($detail[$i]["in_time"] <= $second_late and $detail[$i]["in_time"] >=  $first_late){
                    $slot_status = 1;
                    $slot_color = "#ccff33";
                }
                else{
                ...
                $return1[(int)substr($detail[$i]["Date"],8)] = 
                array($detail[$i]["Date"],date('l', strtotime($detail[$i]["Date"]))
                ,$detail[$i]["in_time"],$detail[$i]["out_time"],$slot_color,$slot_status, substr($detail[$i]["Date"],7)
                );
            ...
            while (strtotime($start_date_of_month) <= strtotime($end_date_of_month)) {
                
                $push = array();
                
                array_push($push, $start_date_of_month );
                array_push($push, date('l', strtotime($start_date_of_month)));
                array_push($push, "");
                array_push($push, "");
                if(date('l', strtotime($start_date_of_month)) == "Sunday"){
                    array_push($push, "Sunday");    
                }
                else{
                    array_push($push, "No_entry");    
                }
            
                array_push($push, "");
                array_push($push, substr($start_date_of_month,8));
                $return2[(int)substr($start_date_of_month,8)] = $push;
                $start_date_of_month = date ("Y-m-d", strtotime("+1 days", strtotime($start_date_of_month)));
                
                
            }
            
            $return = array_replace($return2, $return1);
            


            return response()->json($return);
            // json_encode($current_month_details);
